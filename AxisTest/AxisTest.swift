...
class AxleTest: XCTestCase {
	...
	func testMatrixInitializeWithElements() {
		for n: UInt in 1 ... 10 {
			for m: UInt in 1 ... 10 {
				...
				let matrix = Matrix(elements, rows: n, columns: m, hint: .None, attribute: .EnableLogging)
	...
	func testMatrixInitializeDiagonal() {
		for i: UInt in 1 ... 10 {
			...
			let matrix = Matrix(identity: i, attribute: .EnableLogging)
	...
	func testMatrixCalculationScale() {
		for m: UInt in 1 ... 10 {
			for n: UInt in 1 ... 10 {
				...
				let matrix1 = Matrix(elements1, rows: m, columns: n)
				let matrix2 = Matrix(elements2, rows: m, columns: n)
	...
	func testMatrixCalculationSum() {
		for m: UInt in 1 ... 10 {
			for n: UInt in 1 ... 10 {
				...
				let matrix1 = Matrix(elements1, rows: m, columns: n)
				let matrix2 = Matrix(elements2, rows: m, columns: n)
				let matrix3 = Matrix(elements3, rows: m, columns: n)
	...
	func testMatrixCalculationNorm() {
		for m: UInt in 1 ... 10 {
			for n: UInt in 1 ... 10 {
				...
				let matrix = Matrix(elements, rows: m, columns: n)
