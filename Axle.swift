...
public protocol ElementType: Comparable, CustomStringConvertible {
	static var la_matrix_from_buffer: (UnsafePointer<Self>, la_count_t, la_count_t, la_count_t, la_hint_t, la_attribute_t) -> la_object_t! { get }
	static var la_matrix_to_buffer: (UnsafeMutablePointer<Self>, la_count_t, la_object_t) -> la_status_t  { get }
	static var la_norm: (la_object_t, la_norm_t) -> Self { get }
	static var la_scale: (la_object_t, Self) -> la_object_t! { get }
	static var la_scalar_type: Int32 { get }
	init(_: Double)
}
...
final public class Matrix <T: ElementType> : Equatable {
	...
	public convenience init (_ rows: [T]...) {
		...
		self.init(rows.reduce([], combine: { $0 + $1 }), rows: UInt(rows.count), columns: UInt(rows[0].count))
	}
...
public extension Matrix {
	...
	public var elements: [T] {
		...
		T.la_matrix_to_buffer(&elements, self.columnCount, self._matrix)
		return elements
	}
